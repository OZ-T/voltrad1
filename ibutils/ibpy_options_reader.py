"""
Esta version del reader de IB para opciones utiliza la libreria IBpy
en lugar de la utilizada antes swigibpy
...
class IB_API:
    ...
    def process_messages(self, msg):
        ...
        """
        print ("inicio process_messages type=[%s] field=[%s]" % ( str(msg.typeName), str(msg.values()[1]) ))
        if msg.typeName == "contractDetails":
            ...
            #print self.d_contracts
            #print self.d_opt_contracts
            #print msg
            pass
        else:
            #print msg
...
if __name__ == '__main__':
    ...
    log_file = open( datetime.now().strftime('ib_options_chain_reader_%Y_%m_%d_%H_%M_%S.log'), "w")
    sys.stdout = log_file
    sys.stderr = log_file
